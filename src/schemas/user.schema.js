...
const createUserSchema = Joi.object({
    ...
    surname: Joi.string().trim().min(2).max(100).optional().messages({
...
const updateUserSchema = Joi.object({
    email: Joi.string().trim().min(5).max(100).email().lowercase().messages({
        'string.base': 'El email debe ser un texto',
        'string.email': 'debe ingresar un correo electrónico válido',
    }),
    ...
    surname: Joi.string().trim().min(5).optional().max(100).messages({
        'string.base': 'El apellido debe ser un texto',
        'string.min': 'El campo apellido debe tener al menos 5 carácteres',
    }),
});
